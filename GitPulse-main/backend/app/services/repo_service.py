...
def compare_repos(names: list[str], days: int = 30) -> dict:
    """Return normalized comparison metrics for up to four repositories."""
    names = list(dict.fromkeys(n.strip() for n in names if n.strip()))[:4]
    if not names:
        return {"days": days, "data": []}

    repos = list(get_db()["repo_snapshots"].find(
        {"name": {"$in": names}},
        {"_id": 0, "name": 1, "stars": 1, "forks": 1, "watchers": 1,
         "commits_total": 1, "commits_last_year": 1, "views_all_time": 1,
         "clones_all_time": 1, "language": 1, "html_url": 1},
    ))
    by_name = {r["name"]: r for r in repos}

    # Keep the comparison window deterministic and based on recorded daily data.
    traffic = list(_traffic_col().find(
        {"repo": {"$in": names}},
        {"_id": 0, "repo": 1, "date": 1, "views": 1, "clones": 1,
         "views_uniques": 1, "clones_uniques": 1},
    ).sort("date", -1))
    latest_dates = sorted({d["date"] for d in traffic}, reverse=True)[:days]
    date_set = set(latest_dates)

    traffic_by_repo = {}
    for d in traffic:
        if d["date"] not in date_set:
            continue
        t = traffic_by_repo.setdefault(d["repo"], {"views": 0, "clones": 0, "views_uniques": 0, "clones_uniques": 0})
        for key in t:
            t[key] += d.get(key, 0) or 0
    ...
    for name in names:
        ...
        if not r:
            continue
        t = traffic_by_repo.get(name, {})
        data.append({
            "name": name, "html_url": r.get("html_url", ""), "language": r.get("language", ""),
            "stars": r.get("stars", 0), "forks": r.get("forks", 0), "watchers": r.get("watchers", 0),
            "commits_total": r.get("commits_total", 0), "commits_last_year": r.get("commits_last_year", 0),
            "views_all_time": r.get("views_all_time", 0), "clones_all_time": r.get("clones_all_time", 0),
            "views_window": t.get("views", 0), "clones_window": t.get("clones", 0),
            "views_unique_window": t.get("views_uniques", 0), "clones_unique_window": t.get("clones_uniques", 0),
        })
    return {"days": len(date_set), "data": data}
